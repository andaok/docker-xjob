...
MysqlPasswd = "#mysql_user_password#"
MysqlDB = "#mysql_salt_database#"
# -----------------------------------------
...
def auth_login_new(request):
    ...
    if find_key:
        redis_conn = redis.Redis(host='172.30.33.102',port=6379,decode_responses=True)
        ...
        if res and res.find('username|s:') != -1 and res.find('chinese_name|s:') != -1:
            ...
            conn1 = MySQLdb.connect(host='172.30.33.170',user='cmdb',passwd='cmdb',db='cmdb_job_sys',port=3306)
